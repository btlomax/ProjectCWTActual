...
#define NATIONAL_DEX_RATTATA_ALOLA NATIONAL_DEX_RATTATA
...
#define FOREACH_SPECIES_IN_HOENN_DEX_ORDER(F) \
    ...
    F(MARSHTOMP) \
    F(SWAMPERT) \
    HOENN_DEX_IF(P_GEN_4_POKEMON, F(TURTWIG)) \
    HOENN_DEX_IF(P_GEN_4_POKEMON, F(GROTLE)) \
    HOENN_DEX_IF(P_GEN_4_POKEMON, F(TORTERRA)) \
    HOENN_DEX_IF(P_GEN_4_POKEMON, F(CHIMCHAR)) \
    HOENN_DEX_IF(P_GEN_4_POKEMON, F(MONFERNO)) \
    HOENN_DEX_IF(P_GEN_4_POKEMON, F(INFERNAPE)) \
    HOENN_DEX_IF(P_GEN_4_POKEMON, F(PIPLUP)) \
    HOENN_DEX_IF(P_GEN_4_POKEMON, F(PRINPLUP)) \
    HOENN_DEX_IF(P_GEN_4_POKEMON, F(EMPOLEON)) \
    F(PIDGEY) \
    F(PIDGEOTTO) \
    F(PIDGEOT) \
    ...
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(STARLY)) \
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(STARAVIA)) \
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(STARAPTOR)) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(FLETCHLING)) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(FLETCHINDER)) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(TALONFLAME)) \
    HOENN_DEX_IF(P_GEN_7_POKEMON, F(PIKIPEK)) \
    HOENN_DEX_IF(P_GEN_7_POKEMON, F(TRUMBEAK)) \
    HOENN_DEX_IF(P_GEN_7_POKEMON, F(TOUCANNON)) \
    F(RATTATA_ALOLA) \
    F(POOCHYENA) \
    F(MIGHTYENA) \
    F(ZIGZAGOON) \
    ...
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(LILLIPUP)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(HERDIER)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(STOUTLAND)) \
    F(WURMPLE) \
    F(SILCOON) \
    F(BEAUTIFLY) \
    F(CASCOON) \
    ...
    F(NINJASK) \
    F(SHEDINJA) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(GRUBBIN)) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(CHARJABUG)) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(VIKAVOLT)) \
    HOENN_DEX_IF(P_GEN_1_POKEMON, F(BELLSPROUT)) \
    HOENN_DEX_IF(P_GEN_1_POKEMON, F(WEEPINBELL)) \
    HOENN_DEX_IF(P_GEN_1_POKEMON, F(VICTREEBEL)) \
    F(ODDISH) \
    F(GLOOM) \
    F(VILEPLUME) \
    F(BELLOSSOM) \
    ...
    HOENN_DEX_IF(P_GEN_3_CROSS_EVOS, F(LOTAD)) \
    HOENN_DEX_IF(P_GEN_3_CROSS_EVOS, F(LOMBRE)) \
    HOENN_DEX_IF(P_GEN_3_CROSS_EVOS, F(LUDICOLO)) \
    F(AZURILL) \
    F(MARILL) \
    F(AZUMARILL) \
    HOENN_DEX_IF(P_GEN_1_POKEMON, F(POLIWAG)) \
    HOENN_DEX_IF(P_GEN_1_POKEMON, F(POLIWHIRL)) \
    HOENN_DEX_IF(P_GEN_1_POKEMON, F(POLIWRATH)) \
    HOENN_DEX_IF(P_GEN_2_CROSS_EVOS, F(POLITOED)) \
    F(MAGIKARP) \
    F(GYARADOS) \
    F(PICHU) \
    F(PIKACHU) \
    ...
    F(FLAAFFY) \
    F(AMPHAROS) \
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(SHINX)) \
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(LUXIO)) \
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(LUXRAY)) \
    F(ELECTRIKE) \
    F(MANECTRIC) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(YAMPER)) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(BOLTUND)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(BLITZLE)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(ZEBSTRIKA)) \
    F(RALTS) \
    F(KIRLIA) \
    F(GARDEVOIR) \
    ...
    F(MACHOKE) \
    F(MACHAMP) \
    HOENN_DEX_IF(P_GEN_1_POKEMON, F(MANKEY)) \
    HOENN_DEX_IF(P_GEN_1_POKEMON, F(PRIMEAPE)) \
    HOENN_DEX_IF(P_GEN_9_CROSS_EVOS, F(ANNIHILAPE)) \
    F(ARON) \
    F(LAIRON) \
    F(AGGRON) \
    F(GASTLY) \
    ...
    F(CACNEA) \
    F(CACTURNE) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(DRILBUR)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(EXCADRILL)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(SANDILE)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(KROKOROK)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(KROOKODILE)) \
    HOENN_DEX_IF(P_GEN_2_CROSS_EVOS, F(GLIGAR)) \
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(GLISCOR)) \
    F(GROWLITHE) \
    F(ARCANINE) \
    F(VULPIX) \
    F(NINETALES) \
    ...
    F(MAGCARGO) \
    F(TORKOAL) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(LITWICK)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(LAMPENT)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(CHANDELURE)) \
    HOENN_DEX_IF(P_GEN_9_CROSS_EVOS, F(CHARCADET)) \
    HOENN_DEX_IF(P_GEN_9_CROSS_EVOS, F(CERULEDGE)) \
    F(TENTACOOL) \
    F(TENTACRUEL) \
    F(SLOWPOKE) \
    F(SLOWBRO) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(SLOWKING)) \
    F(PSYDUCK) \
    F(GOLDUCK) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(CLAUNCHER)) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(CLAWITZER)) \
    F(CORPHISH) \
    F(CRAWDAUNT) \
    F(HORSEA) \
    F(SEADRA) \
    ...
    F(VOLTORB) \
    F(ELECTRODE) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(TOXEL)) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(TOXTRICITY)) \
    HOENN_DEX_IF(P_GEN_9_CROSS_EVOS, F(TINKATINK)) \
    HOENN_DEX_IF(P_GEN_9_CROSS_EVOS, F(TINKATUFF)) \
    HOENN_DEX_IF(P_GEN_9_CROSS_EVOS, F(TINKATON)) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(CUFANT)) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(COPPERAJAH)) \
    F(SKARMORY) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(HAWLUCHA)) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(HELIOPTILE)) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(HELIOLISK)) \
    F(SCRAGGY) \
    F(SCRAFTY) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(CLOBBOPUS)) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(GRAPPLOCT)) \
    HOENN_DEX_IF(P_GEN_1_POKEMON, F(FARFETCHD)) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(SIRFETCHD)) \
    HOENN_DEX_IF(P_GEN_7_POKEMON, F(ROCKRUFF)) \
    HOENN_DEX_IF(P_GEN_7_POKEMON, F(LYCANROC)) \
    F(SWABLU) \
    F(ALTARIA) \
    F(NATU) \
    F(XATU) \
    ...
    F(TROPIUS) \
    F(ABSOL) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(ZORUA)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(ZOROARK)) \
    F(PINSIR) \
    F(HERACROSS) \
    HOENN_DEX_IF(P_GEN_2_CROSS_EVOS, F(SCYTHER)) \
    HOENN_DEX_IF(P_GEN_2_CROSS_EVOS, F(SCIZOR)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(BOUFFALANT)) \
    F(TAUROS) \
    F(LILEEP) \
    F(CRADILY) \
    F(ANORITH) \
    F(ARMALDO) \
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(CRANIDOS)) \
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(RAMPARDOS)) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(TYRUNT)) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(TYRANTRUM)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(ARCHEN)) \
    HOENN_DEX_IF(P_GEN_5_POKEMON, F(ARCHEOPS)) \
    F(RHYHORN) \
    F(RHYDON) \
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(RHYPERIOR)) \
    HOENN_DEX_IF(P_GEN_2_CROSS_EVOS, F(SWINUB)) \
    HOENN_DEX_IF(P_GEN_2_CROSS_EVOS, F(PILOSWINE)) \
    HOENN_DEX_IF(P_GEN_4_CROSS_EVOS, F(MAMOSWINE)) \
    F(SNORUNT) \
    F(GLALIE) \
    ...
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(NOIBAT)) \
    HOENN_DEX_IF(P_GEN_6_CROSS_EVOS, F(NOIVERN)) \
    F(DRATINI) \
    F(DRAGONAIR) \
    F(DRAGONITE) \
    F(BAGON) \
    F(SHELGON) \
    F(SALAMENCE) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(DREEPY)) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(DRAKLOAK)) \
    HOENN_DEX_IF(P_GEN_8_CROSS_EVOS, F(DRAGAPULT)) \
    F(BELDUM) \
    F(METANG) \
    F(METAGROSS) \
    F(REGIROCK) \
    F(REGICE) \
    F(REGISTEEL) \
    HOENN_DEX_IF(P_GEN_7_POKEMON, F(ZERAORA)) \
    F(LUGIA) \
    F(HO_OH) \
